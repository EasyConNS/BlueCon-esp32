#pragma once

#define CRC8_POLYNOMIAL  0x07

static inline unsigned char crc8_table_high_first(unsigned char value) {
    unsigned char i, crc;
    crc = value;
    for (i = 8; i > 0; --i)
        if (crc & 0x80)
            crc = (crc << 1) ^ CRC8_POLYNOMIAL;
        else
            crc = (crc << 1);
    return crc;
}

static const unsigned char _crc8_07_table[] = {
        };


static inline void crc8_create_table(unsigned char *table) {
    unsigned short i;
    for (i = 0; i <= 0xFF; i++)
        table[i] = crc8_table_high_first(i);
}

static inline unsigned char crc8(const unsigned char *ptr, int len) {
    unsigned char crc = 0x00;
    while (len--)
        crc = _crc8_07_table[crc ^ *ptr++];
    return (crc);
}
